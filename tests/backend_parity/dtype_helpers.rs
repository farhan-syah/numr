...
pub fn tensor_from_f64<R: Runtime<DType = DType>>(
    ...
    if dtype == DType::F64 {
        return Ok(Tensor::from_slice(data, shape, device).unwrap());
    }
    ...
    let f64_tensor = Tensor::from_slice(data, shape, device).unwrap();
    match client.cast(&f64_tensor, dtype) {
        Ok(t) => Ok(t),
        Err(_) => {
            let f32_data: Vec<f32> = data.iter().map(|&v| v as f32).collect();
            let f32_tensor = Tensor::from_slice(&f32_data, shape, device).unwrap();
            if dtype == DType::F32 {
                Ok(f32_tensor)
            } else {
