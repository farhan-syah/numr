...
fn test_index_select_parity() {
    ...
    for dtype in supported_dtypes("cpu") {
        ...
        let cpu_i = Tensor::try_from_slice(&indices, &[2], &cpu_device).unwrap();
        ...
        if is_dtype_supported("cuda", dtype) {
            with_cuda_backend(|cuda_client, cuda_device| {
                ...
                let i = Tensor::try_from_slice(&indices, &[2], &cuda_device).unwrap();
        ...
        if is_dtype_supported("wgpu", dtype) {
            with_wgpu_backend(|wgpu_client, wgpu_device| {
                ...
                let i = Tensor::try_from_slice(&indices, &[2], &wgpu_device).unwrap();
...
fn test_i32_indices_parity() {
    ...
    for dtype in supported_dtypes("cpu") {
        ...
        let input = tensor_from_f64(&input_data, &[3, 2], dtype, &cpu_device, &cpu_client)
            .unwrap_or_else(|e| panic!("CPU tensor_from_f64 failed for {dtype:?}: {e}"));
        let idx_1d = Tensor::try_from_slice(&[2i32, 0], &[2], &cpu_device).unwrap();
        let idx_2d = Tensor::try_from_slice(&[0i32, 2, 1, 0], &[2, 2], &cpu_device).unwrap();
        ...
        let nd_input = tensor_from_f64(&nd_input_data, &[2, 2], dtype, &cpu_device, &cpu_client)
            .unwrap_or_else(|e| panic!("CPU tensor_from_f64 failed for {dtype:?}: {e}"));
        let nd_idx = Tensor::try_from_slice(&[0i32, 0, 1, 1], &[2, 2], &cpu_device).unwrap();
        ...
        let emb = tensor_from_f64(&emb_data, &[4, 2], dtype, &cpu_device, &cpu_client)
            .unwrap_or_else(|e| panic!("CPU tensor_from_f64 failed for {dtype:?}: {e}"));
        let emb_idx = Tensor::try_from_slice(&[3i32, 0, 1], &[3], &cpu_device).unwrap();
        ...
        let g2d_input = tensor_from_f64(&g2d_input_data, &[3, 3], dtype, &cpu_device, &cpu_client)
            .unwrap_or_else(|e| panic!("CPU tensor_from_f64 failed for {dtype:?}: {e}"));
        let g2d_rows = Tensor::try_from_slice(&[0i32, 1, 2, 0], &[4], &cpu_device).unwrap();
        let g2d_cols = Tensor::try_from_slice(&[0i32, 1, 2, 2], &[4], &cpu_device).unwrap();
        ...
        .unwrap_or_else(|e| panic!("CPU tensor_from_f64 failed for {dtype:?}: {e}"));
        let scatter_reduce_idx = Tensor::try_from_slice(&[0i32, 0, 2], &[3], &cpu_device).unwrap();
        ...
        if is_dtype_supported("cuda", dtype) {
            with_cuda_backend(|cuda_client, cuda_device| {
                ...
                let idx_1d = Tensor::try_from_slice(&[2i32, 0], &[2], &cuda_device).unwrap();
                let idx_2d =
                    Tensor::try_from_slice(&[0i32, 2, 1, 0], &[2, 2], &cuda_device).unwrap();
                ...
                let nd_idx =
                    Tensor::try_from_slice(&[0i32, 0, 1, 1], &[2, 2], &cuda_device).unwrap();
                ...
                let emb = tensor_from_f64(&emb_data, &[4, 2], dtype, &cuda_device, &cuda_client)
                    .unwrap_or_else(|e| panic!("CUDA tensor_from_f64 failed for {dtype:?}: {e}"));
                let emb_idx = Tensor::try_from_slice(&[3i32, 0, 1], &[3], &cuda_device).unwrap();
                ...
                let g2d_rows =
                    Tensor::try_from_slice(&[0i32, 1, 2, 0], &[4], &cuda_device).unwrap();
                let g2d_cols =
                    Tensor::try_from_slice(&[0i32, 1, 2, 2], &[4], &cuda_device).unwrap();
                ...
                .unwrap_or_else(|e| panic!("CUDA tensor_from_f64 failed for {dtype:?}: {e}"));
                let scatter_reduce_idx =
                    Tensor::try_from_slice(&[0i32, 0, 2], &[3], &cuda_device).unwrap();
                let scatter_reduce_src = tensor_from_f64(
                    &scatter_reduce_src_data,
                    &[3],
                    dtype,
        ...
        if is_dtype_supported("wgpu", dtype) {
            with_wgpu_backend(|wgpu_client, wgpu_device| {
                ...
                let idx_1d = Tensor::try_from_slice(&[2i32, 0], &[2], &wgpu_device).unwrap();
                let idx_2d =
                    Tensor::try_from_slice(&[0i32, 2, 1, 0], &[2, 2], &wgpu_device).unwrap();
                ...
                let nd_idx =
                    Tensor::try_from_slice(&[0i32, 0, 1, 1], &[2, 2], &wgpu_device).unwrap();
                ...
                let emb = tensor_from_f64(&emb_data, &[4, 2], dtype, &wgpu_device, &wgpu_client)
                    .unwrap_or_else(|e| panic!("WGPU tensor_from_f64 failed for {dtype:?}: {e}"));
                let emb_idx = Tensor::try_from_slice(&[3i32, 0, 1], &[3], &wgpu_device).unwrap();
                ...
                let g2d_rows =
                    Tensor::try_from_slice(&[0i32, 1, 2, 0], &[4], &wgpu_device).unwrap();
                let g2d_cols =
                    Tensor::try_from_slice(&[0i32, 1, 2, 2], &[4], &wgpu_device).unwrap();
                ...
                .unwrap_or_else(|e| panic!("WGPU tensor_from_f64 failed for {dtype:?}: {e}"));
                let scatter_reduce_idx =
                    Tensor::try_from_slice(&[0i32, 0, 2], &[3], &wgpu_device).unwrap();
                let scatter_reduce_src = tensor_from_f64(
                    &scatter_reduce_src_data,
                    &[3],
                    dtype,
...
fn test_gather_scatter_parity() {
    ...
    for dtype in supported_dtypes("cpu") {
        ...
        let cpu_x = tensor_from_f64(&input_data, &[3, 2], dtype, &cpu_device, &cpu_client)
            .unwrap_or_else(|e| panic!("CPU tensor_from_f64 failed for {dtype:?}: {e}"));
        let cpu_i = Tensor::try_from_slice(&gather_indices, &[2, 2], &cpu_device).unwrap();
        let cpu_gather = cpu_client
        ...
        if is_dtype_supported("cuda", dtype) {
            with_cuda_backend(|cuda_client, cuda_device| {
                let x = tensor_from_f64(&input_data, &[3, 2], dtype, &cuda_device, &cuda_client)
                    .unwrap_or_else(|e| panic!("CUDA tensor_from_f64 failed for {dtype:?}: {e}"));
                let i = Tensor::try_from_slice(&gather_indices, &[2, 2], &cuda_device).unwrap();
                let result_gather = cuda_client
                    .gather(&x, 0, &i)
                    .unwrap_or_else(|e| panic!("CUDA gather failed for {dtype:?}: {e}"));
                assert_tensor_allclose(
        ...
        if is_dtype_supported("wgpu", dtype) {
            with_wgpu_backend(|wgpu_client, wgpu_device| {
                let x = tensor_from_f64(&input_data, &[3, 2], dtype, &wgpu_device, &wgpu_client)
                    .unwrap_or_else(|e| panic!("WGPU tensor_from_f64 failed for {dtype:?}: {e}"));
                let i = Tensor::try_from_slice(&gather_indices, &[2, 2], &wgpu_device).unwrap();
                let result_gather = wgpu_client
                    .gather(&x, 0, &i)
                    .unwrap_or_else(|e| panic!("WGPU gather failed for {dtype:?}: {e}"));
                assert_tensor_allclose(
...
fn test_gather_nd_bincount_embedding_parity() {
    ...
    for dtype in supported_dtypes("cpu") {
        ...
        let input = tensor_from_f64(&input_data, &[2, 2], dtype, &cpu_device, &cpu_client)
            .unwrap_or_else(|e| panic!("CPU tensor_from_f64 failed for {dtype:?}: {e}"));
        let nd_idx = Tensor::try_from_slice(&nd_indices_i32, &[2, 2], &cpu_device).unwrap();
        let cpu_nd = cpu_client
            .gather_nd(&input, &nd_idx)
            .unwrap_or_else(|e| panic!("CPU gather_nd failed for {dtype:?}: {e}"));

        // bincount operates on i64 indices, returns i64 counts (not parameterized)
        let bins_input = Tensor::try_from_slice(&bins_input_i64, &[7], &cpu_device).unwrap();
        ...
        let emb = tensor_from_f64(&emb_data, &[4, 2], dtype, &cpu_device, &cpu_client)
            .unwrap_or_else(|e| panic!("CPU tensor_from_f64 failed for {dtype:?}: {e}"));
        let emb_idx = Tensor::try_from_slice(&emb_idx_i64, &[3], &cpu_device).unwrap();
        let cpu_emb = cpu_client
            .embedding_lookup(&emb, &emb_idx)
            .unwrap_or_else(|e| panic!("CPU embedding_lookup failed for {dtype:?}: {e}"));
        ...
        if is_dtype_supported("cuda", dtype) {
            with_cuda_backend(|cuda_client, cuda_device| {
                let x = tensor_from_f64(&input_data, &[2, 2], dtype, &cuda_device, &cuda_client)
                    .unwrap_or_else(|e| panic!("CUDA tensor_from_f64 failed for {dtype:?}: {e}"));
                let i = Tensor::try_from_slice(&nd_indices_i32, &[2, 2], &cuda_device).unwrap();
                ...
                let b_in = Tensor::try_from_slice(&bins_input_i64, &[7], &cuda_device).unwrap();
                ...
                let e = tensor_from_f64(&emb_data, &[4, 2], dtype, &cuda_device, &cuda_client)
                    .unwrap_or_else(|e| panic!("CUDA tensor_from_f64 failed for {dtype:?}: {e}"));
                let ei = Tensor::try_from_slice(&emb_idx_i64, &[3], &cuda_device).unwrap();
                let result_emb = cuda_client
        ...
        if is_dtype_supported("wgpu", dtype) {
            with_wgpu_backend(|wgpu_client, wgpu_device| {
                let x = tensor_from_f64(&input_data, &[2, 2], dtype, &wgpu_device, &wgpu_client)
                    .unwrap_or_else(|e| panic!("WGPU tensor_from_f64 failed for {dtype:?}: {e}"));
                let i = Tensor::try_from_slice(&nd_indices_i32, &[2, 2], &wgpu_device).unwrap();
                ...
                let b_in = Tensor::try_from_slice(&bins_input_i64, &[7], &wgpu_device).unwrap();
                ...
                let e = tensor_from_f64(&emb_data, &[4, 2], dtype, &wgpu_device, &wgpu_client)
                    .unwrap_or_else(|e| panic!("WGPU tensor_from_f64 failed for {dtype:?}: {e}"));
                let ei = Tensor::try_from_slice(&emb_idx_i64, &[3], &wgpu_device).unwrap();
                let result_emb = wgpu_client
                    .embedding_lookup(&e, &ei)
                    .unwrap_or_else(|e| panic!("WGPU embedding_lookup failed for {dtype:?}: {e}"));
                assert_tensor_allclose(
...
fn test_scatter_reduce_sum_parity() {
    ...
    for dtype in supported_dtypes("cpu") {
        ...
        let dst = tensor_from_f64(&dst_data, &[4], dtype, &cpu_device, &cpu_client)
            .unwrap_or_else(|e| panic!("CPU tensor_from_f64 failed for {dtype:?}: {e}"));
        let idx = Tensor::try_from_slice(&indices, &[3], &cpu_device).unwrap();
        ...
        if is_dtype_supported("cuda", dtype) {
            with_cuda_backend(|cuda_client, cuda_device| {
                let d = tensor_from_f64(&dst_data, &[4], dtype, &cuda_device, &cuda_client)
                    .unwrap_or_else(|e| panic!("CUDA tensor_from_f64 failed for {dtype:?}: {e}"));
                let i = Tensor::try_from_slice(&indices, &[3], &cuda_device).unwrap();
        ...
        if is_dtype_supported("wgpu", dtype) {
            with_wgpu_backend(|wgpu_client, wgpu_device| {
                let d = tensor_from_f64(&dst_data, &[4], dtype, &wgpu_device, &wgpu_client)
                    .unwrap_or_else(|e| panic!("WGPU tensor_from_f64 failed for {dtype:?}: {e}"));
                let i = Tensor::try_from_slice(&indices, &[3], &wgpu_device).unwrap();
...
fn test_scatter_reduce_2d_inner_parity() {
    ...
    let cpu_idx = Tensor::try_from_slice(&idx_data, &[2, 3], &cpu_device).unwrap();
    let cpu_src =
    ...
    if is_dtype_supported("cuda", DType::F32) {
        with_cuda_backend(|cuda_client, cuda_device| {
            let dst = tensor_from_f64(&dst_data, &[4, 3], DType::F32, &cuda_device, &cuda_client)
                .unwrap();
            let idx = Tensor::try_from_slice(&idx_data, &[2, 3], &cuda_device).unwrap();
    ...
    if is_dtype_supported("wgpu", DType::F32) {
        with_wgpu_backend(|wgpu_client, wgpu_device| {
            let dst = tensor_from_f64(&dst_data, &[4, 3], DType::F32, &wgpu_device, &wgpu_client)
                .unwrap();
            let idx = Tensor::try_from_slice(&idx_data, &[2, 3], &wgpu_device).unwrap();
