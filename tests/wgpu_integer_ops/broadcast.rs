// Broadcasting behavior across ranks and directions (F32/I32).

use crate::common::backend_lock::with_wgpu_backend_or_skip;
...
fn test_broadcast_3d() {
    with_wgpu_backend_or_skip(|client, device| {
        ...
        .unwrap();
        let b =
            Tensor::<WgpuRuntime>::from_slice(&[10.0f32, 20.0, 30.0, 40.0], &[4], &device).unwrap();
...
fn test_broadcast_max() {
    with_wgpu_backend_or_skip(|client, device| {
        // max([2, 3], [3]) -> [2, 3]
        let a =
            Tensor::<WgpuRuntime>::from_slice(&[1.0f32, 5.0, 3.0, 4.0, 2.0, 6.0], &[2, 3], &device)
                .unwrap();
...
fn test_broadcast_i32_add() {
    with_wgpu_backend_or_skip(|client, device| {
        // [2, 3] + [3] -> [2, 3]
        let a =
            Tensor::<WgpuRuntime>::from_slice(&[1i32, 2, 3, 4, 5, 6], &[2, 3], &device).unwrap();
