...
mod tests {
    ...
    fn test_meshgrid_2d_ij() {
        ...
        let y = Tensor::<CpuRuntime>::from_slice(&[4.0f32, 5.0], &[2], &device).unwrap();
    ...
    fn test_meshgrid_3d() {
        ...
        let b = Tensor::<CpuRuntime>::from_slice(&[3.0f32, 4.0, 5.0], &[3], &device).unwrap();
        let c = Tensor::<CpuRuntime>::from_slice(&[6.0f32, 7.0], &[2], &device).unwrap();
