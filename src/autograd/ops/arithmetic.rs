...
use crate::autograd::{GradFn, Var, var_div, var_log, var_mul, var_neg};
use crate::error::Result;
use crate::ops::{BinaryOps, TensorOps, UnaryOps};
...
mod tests {
    ...
    fn test_add_backward_same_shape() {
        ...
        let grad_out = Tensor::<CpuRuntime>::from_slice(&[1.0f32, 1.0], &[2], &device).unwrap();
    ...
    fn test_mul_backward() {
        ...
        // dz/da = b = 3
        // dz/db = a = 2
        let a = Tensor::<CpuRuntime>::from_slice(&[2.0f32], &[1], &device).unwrap();
        let b = Tensor::<CpuRuntime>::from_slice(&[3.0f32], &[1], &device).unwrap();
    ...
    fn test_div_backward() {
        ...
        // dz/da = 1/b = 0.5
        // dz/db = -a/b² = -6/4 = -1.5
        let a = Tensor::<CpuRuntime>::from_slice(&[6.0f32], &[1], &device).unwrap();
