...
mod tests {
    ...
    fn test_dual_add() {
        ...
        let a_tangent =
            Tensor::<CpuRuntime>::from_slice(&[1.0f32, 0.0, 0.0], &[3], &device).unwrap();
        let a = DualTensor::with_tangent(a_primal, a_tangent);

        let b_primal =
            Tensor::<CpuRuntime>::from_slice(&[4.0f32, 5.0, 6.0], &[3], &device).unwrap();
        let b_tangent =
            Tensor::<CpuRuntime>::from_slice(&[0.0f32, 1.0, 0.0], &[3], &device).unwrap();
    ...
    fn test_dual_mul() {
        ...
        let x_primal = Tensor::<CpuRuntime>::from_slice(&[3.0f32], &[1], &device).unwrap();
