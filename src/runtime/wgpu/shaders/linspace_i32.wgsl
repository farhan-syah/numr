...
// that evaluation still goes through numr_f32_to_i32_sat, so a value past the
// dtype's range clamps the way CPU's `as i32` does instead of leaving the
...
@group(0) @binding(0) var<storage, read_write> linspace_out: array<i32>;
@group(0) @binding(1) var<uniform> linspace_params: LinspaceIntParams;

@compute @workgroup_size(256)
fn linspace_i32(@builtin(global_invocation_id) gid: vec3<u32>) {
    let idx = gid.x;
    ...
    if (linspace_params.exact != 0u) {
        ...
        linspace_out[idx] = numr_i64_to_i32_sat(numr_i64_div_u32_trunc(numer, linspace_params.divisor));
        return;
    }
    ...
    linspace_out[idx] = numr_f32_to_i32_sat(value);
}
