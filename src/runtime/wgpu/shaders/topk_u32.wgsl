// Top-k operations for u32. Ordering helpers are prepended from sort_cmp.rs.
...
@group(0) @binding(0) var<storage, read_write> topk_input: array<u32>;
@group(0) @binding(1) var<storage, read_write> topk_values: array<u32>;
...
fn topk_u32(
    ...
    for (var i = tid; i < n; i = i + WORKGROUP_SIZE) {
        if (i < sort_size) {
            ...
            shared_vals[i] = sort_pad_u32(largest);
            shared_idxs[i] = i32(i);
        }
    }
    workgroupBarrier();
    ...
    for (var k_: u32 = 2u; k_ <= n; k_ = k_ << 1u) {
        for (var j: u32 = k_ >> 1u; j > 0u; j = j >> 1u) {
            for (var i = tid; i < (n >> 1u); i = i + WORKGROUP_SIZE) {
                ...
                if (ij_pair < n) {
                    bitonic_cas_u32(ij, ij_pair, ascending_local, largest);
                }
            }
            workgroupBarrier();
        }
