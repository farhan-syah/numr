// Widen a U32 tensor into the 64-bit scatter accumulator.
//
// Concatenated after int_saturate.wgsl. Runs before scatter_wide_u32.wgsl's
...
@group(0) @binding(0) var<storage, read_write> sw_seed_src: array<u32>;
...
fn scatter_wide_seed_u32(@builtin(global_invocation_id) gid: vec3<u32>) {
    let idx = gid.x;
    if (idx >= sw_seed_params.n) {
        return;
    }
    let v = sw_seed_src[idx];
    sw_seed_acc[idx * 2u] = v;
    sw_seed_acc[idx * 2u + 1u] = 0u;
}
