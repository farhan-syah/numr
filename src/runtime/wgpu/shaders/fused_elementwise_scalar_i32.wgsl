// Fused elementwise scalar WGSL shader (I32)
...
@group(0) @binding(0) var<storage, read_write> sfma_a: array<i32>;
@group(0) @binding(1) var<storage, read_write> sfma_out: array<i32>;
@group(0) @binding(2) var<uniform> sfma_params: ScalarFmaParams;

@compute @workgroup_size(256)
fn fused_mul_add_scalar_i32(@builtin(global_invocation_id) gid: vec3<u32>) {
    let idx = gid.x;
