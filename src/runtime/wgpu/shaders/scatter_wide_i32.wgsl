// Wide-accumulator scatter sum and its narrowing epilogue, for I32.
//
...
@group(0) @binding(0) var<storage, read_write> sw_src: array<i32>;
...
fn scatter_wide_sum_i32(@builtin(global_invocation_id) gid: vec3<u32>) {
    let idx = gid.x;
...
@group(0) @binding(2) var<storage, read_write> sw_fin_out: array<i32>;
@group(0) @binding(3) var<uniform> sw_fin_params: ScatterWideParams;

@compute @workgroup_size(256)
fn scatter_wide_finalize_i32(@builtin(global_invocation_id) gid: vec3<u32>) {
    let idx = gid.x;
    ...
    sw_fin_out[idx] = numr_i64_to_i32_sat(acc);
}
