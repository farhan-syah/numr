// Semiring matmul: min_plus for i32
// C[i,j] = min_k( A[i,k] + B[k,j] )
// Entry points: semiring_matmul_min_plus_i32, batched_semiring_matmul_min_plus_i32
//
// Concatenated after int_saturate.wgsl, which defines NUMR_I32_MAX / NUMR_I32_MIN.
// The reduce identity is i32::MAX: CPU's `reduce_identity` casts +inf to the element
...
fn semiring_matmul_min_plus_i32(
...
@compute @workgroup_size(16, 16, 1)
fn batched_semiring_matmul_min_plus_i32(
    @builtin(global_invocation_id) global_id: vec3<u32>
) {
    let M = sr_params.M;
    let K = sr_params.K;
