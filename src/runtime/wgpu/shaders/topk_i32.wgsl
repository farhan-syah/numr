// Top-k operations for i32. Ordering helpers are prepended from sort_cmp.rs.
...
@group(0) @binding(0) var<storage, read_write> topk_input: array<i32>;
@group(0) @binding(1) var<storage, read_write> topk_values: array<i32>;
...
fn bitonic_cas_i32(i: u32, j: u32, ascending_local: bool, descending: bool) {
    ...
    let swap = select(
        sort_rank_less_i32(vi, ii, vj, ij, descending),
        sort_rank_less_i32(vj, ij, vi, ii, descending),
        ascending_local
    );
    if (swap) {
        shared_vals[i] = vj;
...
fn topk_i32(
    ...
    for (var i = tid; i < n; i = i + WORKGROUP_SIZE) {
        if (i < sort_size) {
            ...
            shared_vals[i] = sort_pad_i32(largest);
            shared_idxs[i] = i32(i);
        }
    }
    workgroupBarrier();
    ...
    for (var k_: u32 = 2u; k_ <= n; k_ = k_ << 1u) {
        for (var j: u32 = k_ >> 1u; j > 0u; j = j >> 1u) {
            for (var i = tid; i < (n >> 1u); i = i + WORKGROUP_SIZE) {
                ...
                if (ij_pair < n) {
                    bitonic_cas_i32(ij, ij_pair, ascending_local, largest);
                }
            }
            workgroupBarrier();
        }
