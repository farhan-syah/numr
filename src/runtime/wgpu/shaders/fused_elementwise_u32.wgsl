// Fused elementwise WGSL shaders (U32)
// fused_mul_add: out = a * b + c
// fused_add_mul: out = (a + b) * c
//
// The two arithmetic operators are the plain ones, exactly as binary_u32.wgsl
...
@group(0) @binding(0) var<storage, read_write> tern_a: array<u32>;
@group(0) @binding(1) var<storage, read_write> tern_b: array<u32>;
@group(0) @binding(2) var<storage, read_write> tern_c: array<u32>;
@group(0) @binding(3) var<storage, read_write> tern_out: array<u32>;
@group(0) @binding(4) var<uniform> tern_params: TernaryParams;

@compute @workgroup_size(256)
fn fused_mul_add_u32(@builtin(global_invocation_id) gid: vec3<u32>) {
    let idx = gid.x;
...
@compute @workgroup_size(256)
fn fused_add_mul_u32(@builtin(global_invocation_id) gid: vec3<u32>) {
    let idx = gid.x;
    if (idx < tern_params.numel) {
        tern_out[idx] = (tern_a[idx] + tern_b[idx]) * tern_c[idx];
    }
