// Semiring matmul: plus_max for i32
// C[i,j] = sum_k( max(A[i,k], B[k,j]) )
// Entry points: semiring_matmul_plus_max_i32, batched_semiring_matmul_plus_max_i32
//
// Concatenated after int_saturate.wgsl like its sibling semirings, though this
// one needs no constant from it. The reduce identity is 0, the additive identity.
...
fn sr_reduce(acc: i32, val: i32) -> i32 {
    return sr_wrap_add(acc, val);
}

@compute @workgroup_size(16, 16, 1)
fn semiring_matmul_plus_max_i32(
...
@compute @workgroup_size(16, 16, 1)
fn batched_semiring_matmul_plus_max_i32(
    @builtin(global_invocation_id) global_id: vec3<u32>
) {
    let M = sr_params.M;
    let K = sr_params.K;
