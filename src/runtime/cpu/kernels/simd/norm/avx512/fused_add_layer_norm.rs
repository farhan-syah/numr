...
pub unsafe fn fused_add_layer_norm_f32(
    ...
    for batch in 0..batch_size {
        ...
        for c in 0..chunks {
            ...
            sum_acc = _mm512_add_ps(sum_acc, _mm512_sub_ps(pn, v_ref));
        }
        ...
        for c in 0..chunks {
            ...
            let pn = _mm512_sub_ps(_mm512_loadu_ps(pre_norm.add(offset)), v_ref);
            let v_weight = _mm512_loadu_ps(weight.add(w_offset));
            let v_bias = _mm512_loadu_ps(bias.add(w_offset));

            let diff = _mm512_sub_ps(pn, v_shifted_mean);
...
pub unsafe fn fused_add_layer_norm_f64(
    ...
    for batch in 0..batch_size {
        ...
        for c in 0..chunks {
            ...
            sum_acc = _mm512_add_pd(sum_acc, _mm512_sub_pd(pn, v_ref));
        }
        ...
        for c in 0..chunks {
            ...
            let pn = _mm512_sub_pd(_mm512_loadu_pd(pre_norm.add(offset)), v_ref);
            let v_weight = _mm512_loadu_pd(weight.add(w_offset));
            let v_bias = _mm512_loadu_pd(bias.add(w_offset));

            let diff = _mm512_sub_pd(pn, v_shifted_mean);
...
pub unsafe fn fused_add_layer_norm_bwd_f32(
    ...
    for batch in 0..batch_size {
        ...
        let v_inv_std = _mm512_set1_ps(inv_std);
        let v_shifted_mean_gs = _mm512_set1_ps(mean_gs);
        let v_shifted_mean_gs_n = _mm512_set1_ps(mean_gs_n);
        ...
        for c in 0..chunks {
            ...
            let normalized = _mm512_mul_ps(_mm512_sub_ps(pn, v_shifted_mean), v_inv_std);
            let gs = _mm512_mul_ps(g, w);
            let d_ir = _mm512_mul_ps(
                ...
                _mm512_sub_ps(
                    ...
                    _mm512_add_ps(
                        v_shifted_mean_gs,
                        _mm512_mul_ps(normalized, v_shifted_mean_gs_n),
                    ),
                ),
            );
...
pub unsafe fn fused_add_layer_norm_bwd_f64(
    ...
    for batch in 0..batch_size {
        ...
        let v_inv_std = _mm512_set1_pd(inv_std);
        let v_shifted_mean_gs = _mm512_set1_pd(mean_gs);
        let v_shifted_mean_gs_n = _mm512_set1_pd(mean_gs_n);
        ...
        for c in 0..chunks {
            ...
            let normalized = _mm512_mul_pd(_mm512_sub_pd(pn, v_shifted_mean), v_inv_std);
            let gs = _mm512_mul_pd(g, w);
            let d_ir = _mm512_mul_pd(
                ...
                _mm512_sub_pd(
                    ...
                    _mm512_add_pd(
                        v_shifted_mean_gs,
                        _mm512_mul_pd(normalized, v_shifted_mean_gs_n),
                    ),
                ),
            );
