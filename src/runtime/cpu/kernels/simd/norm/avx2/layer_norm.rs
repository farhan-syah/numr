...
pub unsafe fn layer_norm_f32(
    ...
    for batch in 0..batch_size {
        ...
        for c in 0..chunks {
            let v = _mm256_sub_ps(_mm256_loadu_ps(input.add(row_start + c * F32_LANES)), v_ref);
        ...
        for c in 0..chunks {
            let v = _mm256_sub_ps(_mm256_loadu_ps(input.add(row_start + c * F32_LANES)), v_ref);
            let diff = _mm256_sub_ps(v, v_shifted_mean);
        ...
        for c in 0..chunks {
            ...
            let diff = _mm256_sub_ps(_mm256_sub_ps(v_input, v_ref), v_shifted_mean);
...
pub unsafe fn layer_norm_f64(
    ...
    for batch in 0..batch_size {
        ...
        for c in 0..chunks {
            let v = _mm256_sub_pd(_mm256_loadu_pd(input.add(row_start + c * F64_LANES)), v_ref);
        ...
        for c in 0..chunks {
            let v = _mm256_sub_pd(_mm256_loadu_pd(input.add(row_start + c * F64_LANES)), v_ref);
            let diff = _mm256_sub_pd(v, v_shifted_mean);
        ...
        for c in 0..chunks {
            ...
            let diff = _mm256_sub_pd(_mm256_sub_pd(v_input, v_ref), v_shifted_mean);
