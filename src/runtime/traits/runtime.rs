...
pub trait Runtime: Clone + Send + Sync + 'static {
    ...
    ///
    /// ```ignore
    /// let a = Tensor::<CudaRuntime>::from_slice(&[1.0f32, 2.0], &[2], device).unwrap();
    /// let b = Tensor::<CudaRuntime>::from_slice(&[3.0f32, 4.0], &[2], device).unwrap();
    /// let c = Tensor::<CudaRuntime>::zeros(&[2], DType::F32, device).unwrap();
    ///
