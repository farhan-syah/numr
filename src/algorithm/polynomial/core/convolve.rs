...
/// ```ignore
/// // (1 + 2x) * (3 + 4x) = 3 + 10x + 8x²
/// let a = Tensor::from_slice(&[1.0f32, 2.0], &[2], &device).unwrap();
/// let b = Tensor::from_slice(&[3.0f32, 4.0], &[2], &device).unwrap();
        ...
        + ComplexOps<R>,
{
    ...
    // Handle edge cases
    if n_a == 0 || n_b == 0 {
        return Tensor::zeros(&[0], dtype, device);
    }
...
mod tests {
    ...
    fn test_convolve_direct_asymmetric() {
        ...
        let b = Tensor::<CpuRuntime>::from_slice(&[3.0f32, 4.0], &[2], &device).unwrap();
    ...
    fn test_convolve_impl_selects_fft() {
        ...
        let a = Tensor::<CpuRuntime>::from_slice(&a_data, &[10], &device).unwrap();
        let b = Tensor::<CpuRuntime>::from_slice(&b_data, &[10], &device).unwrap();
    ...
    fn test_convolve_direct_vs_fft_equivalence() {
        ...
        let b = Tensor::<CpuRuntime>::from_slice(&[5.0f32, 6.0, 7.0, 8.0], &[4], &device).unwrap();
    ...
    fn test_convolve_f64() {
        ...
        let a = Tensor::<CpuRuntime>::from_slice(&[1.0f64, 2.0, 3.0], &[3], &device).unwrap();
        let b = Tensor::<CpuRuntime>::from_slice(&[4.0f64, 5.0], &[2], &device).unwrap();
    ...
    fn test_convolve_empty() {
        ...
        let a = Tensor::<CpuRuntime>::from_slice(&[1.0f32, 2.0], &[2], &device).unwrap();
        let b = Tensor::<CpuRuntime>::from_slice(&[] as &[f32], &[0], &device).unwrap();
